...
class Function():
    ...
    def get_result(self):
        ...
        if not valid_rules:
            ...
            return
        spacer = " " * 4
        result_str = "// rule auto-generated by FuzzyYara\n"
        result_str += f"rule {self.name} " + "\n{\n"
        ...
        try:
            if has_address:
                ...
                if "address" in [i.type.name.lower() for i in inst]:
                    log_info(f"Found possible missed instruction: {"".join(str(i) for i in inst)}", "FuzzyYara")
    ...
    while (curr_addr < end):
        ...
        for instruction in block:
            ...
            if curr_addr < start:
                ...
                log_info(f"skipping instruction '{"".join(i.text for i in instruction[0])}' at address {hex(curr_addr)}", "FuzzyYara")
                curr_addr += inst_size
                continue
